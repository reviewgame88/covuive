...
//        }
    ...
    jQuery("head").append("<script src='/ChineChess/screenshot.js'></script>"); 
...
function AsyncGetAutoMove(banarr) {
	...
	xmlhttpMove.open('POST', '/ChineChess/api/get_move.php', true);
	xmlhttpMove.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
	xmlhttpMove.onreadystatechange = function() {
		if (xmlhttpMove.readyState == 4) {
			if (xmlhttpMove.status == 200) {
...
function AsyncUpdateMoves() {
    ...
    xmlhttp.open('GET', '/ChineChess/api/get_move.php?act=get_all&fen=' + temp_fen, true);
...
function AutoMove() {
    ...
    setTimeout(function(){ 
    	...
    	if (curstep >= 4) {
    		...
    		xmlhttpAuto.open('POST', '/ChineChess/api/get_move.php', true);
